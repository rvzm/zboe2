#!/usr/bin/env bash
# zboe2 installer — Ubuntu/Debian (apt)
#
...
Detected httpd: nginx
  Proxy zboe2 through it with a server block like
  (/etc/nginx/sites-available/zboe.conf, symlinked into sites-enabled/):
...
Detected httpd: Apache
  Enable the proxy modules, then add a vhost:

    sudo a2enmod proxy proxy_http

  /etc/apache2/sites-available/zboe.conf:
  ...
  Apply with: sudo a2ensite zboe && sudo systemctl reload apache2
EOF
    fi
